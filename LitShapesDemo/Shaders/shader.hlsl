...
cbuffer cbPerObject : register(b1)
{
	...
	float4x4 Pad; // Compatibility issue, struct updated for other demo
...
float4 ComputeFoggedColor(float4 litColor, float distToEye)
{
	...
	const float fogStartDist = 200;
	const float fullFogDist = 500;
...
float4 PS(VertexOut pin) : SV_Target
{
	...
	{
		litColor = gMaterial.Diffuse.rgba + spec; //ambient + diffuse + spec;
		//litColor = ComputeFoggedColor(litColor, distToEye);
