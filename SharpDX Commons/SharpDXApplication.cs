...
namespace SharpDXCommons
{
	...
	{
		protected const DeviceCreationFlags DEVICE_CREATION_FLAGS = DeviceCreationFlags.None;
		protected const SwapChainFlags SWAP_CHAIN_FLAGS = SwapChainFlags.AllowModeSwitch;
		protected const Format DEPTH_STENCIL_FORMAT = Format.D24_UNorm_S8_UInt;

		protected Color4 BackgroundColor = new Color4(0.4f, 0.5f, 0.7f, 1.0f);
		...
		protected bool IsFullscreenWindow
		{
			get { return RenderWindow.Fullscreen; }
		}
		...
		private void InitDevice()
		{
			SwapChainDescription SwapChainDesc = new SwapChainDescription
			{
				...
				IsWindowed = true, // Fullscreen mode will be set later, when the rendering window is properly set up
			...
			D3D11Device.CreateWithSwapChain(Configuration.Adapter, DEVICE_CREATION_FLAGS, SwapChainDesc, out Device, out SwapChain);
		...
		{
			RenderTargetTexture = Texture2D.FromSwapChain<Texture2D>(SwapChain, 0);
			BackbufferRTV = new RenderTargetView(Device, RenderTargetTexture);
			...
			Context.OutputMerger.SetRenderTargets(DepthStencilView, BackbufferRTV);
