...
namespace SharpDXCommons.Cameras
{
	public class OrbitalControls
	{
		...
		public OrbitalControls(RenderWindow window, Vector3 pos, float radiusMin, float radiusMax, float radius, bool leftButton) 
			: this(window, pos, radiusMin, radiusMax, radius)
		{
		...
		{
			if (e.Button == (LeftButton ? MouseButtons.Left : MouseButtons.Right))
			{
		...
		private void OnMouseWheel(object sender, MouseEventArgs e)
		{
			...
			Radius = MathUtil.Clamp(Radius, RadiusMin, RadiusMax);
		}
		...
		{
			return Vector3.Normalize(Position - GetCameraPosition());
		}
